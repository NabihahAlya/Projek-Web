...
class Admin extends CI_Controller {
    ...
    public function get_fasilitas_by_id()
    {
        ...
        if ($fasilitas) {
            // Ambil data foto hanya untuk tipe popup
            $foto_data = ($fasilitas->tipe_aksi == 'popup') 
                ? $this->Fasilitas_model->get_foto_by_fasilitas($id) 
                : [];
    
            // Format response berdasarkan tipe aksi
            $response = [
                'id_fasilitas' => $fasilitas->id_fasilitas,
                'nama' => $fasilitas->nama,
                'ikon' => $fasilitas->ikon,
                'tipe_aksi' => $fasilitas->tipe_aksi
            ];
    
            // Tambahkan field sesuai tipe aksi
            if ($fasilitas->tipe_aksi == 'link') {
                $response['link'] = $fasilitas->link ?? '';
                $response['deskripsi'] = '';
            } else {
                $response['deskripsi'] = $fasilitas->deskripsi ?? '';
                $response['fotos'] = $foto_data;
            }
    
            // Pastikan output clean
            ob_clean();
