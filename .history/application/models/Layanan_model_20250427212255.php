...
class Layanan_model extends CI_Model {
    ...
    public function get_by_id($id)
    {
        ...
        $this->db->select('id_layanan, nama, ikon, tipe_aksi, link, deskripsi');
        $this->db->where('id_layanan', $id);
        return $this->db->get('layanan')->row();
    }
    
    public function get_foto_by_layanan($id_layanan)
    {
        // Pastikan return array kosong jika tidak ada data
        $this->db->select('foto'); // Pastikan nama kolom sesuai
        $this->db->where('id_layanan', $id_layanan);
        $result = $this->db->get('layanan_foto')->result_array();
        return $result ?: [];
    }
