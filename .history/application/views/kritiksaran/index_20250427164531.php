...
<html lang="en">
  ...
  <body>
    ...
    <div class="form">
    <?php 
// Hanya tampilkan jika ada flashdata 'alert_kritik'
if ($this->session->flashdata('alert_kritik')) : 
?>
    <div class="alert" style="padding: 10px; background: #4CAF50; color: white; margin-bottom: 15px; border-radius: 5px; text-align: center;">
        <?= $this->session->flashdata('alert_kritik') ?>
    </div>
    <?php 
    // Hapus manual setelah ditampilkan
    $this->session->unset_flashdata('alert_kritik'); 
    ?>
<?php endif; ?>
      <h2>Kritik & Saran</h2>
      <form action="<?= base_url('beranda/simpan_kritik')?>" method="POST">
        
      <?php echo validation_errors('<div class="error" style="color:red; margin-bottom:15px;">', '</div>'); ?>
